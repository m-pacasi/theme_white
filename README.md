...
## Table of Contents
   ...
   * [Fury - Angular 8+ Material Design Admin Template](#fury---angular-6+-material-design-admin-template)
...
| `src`                 | Contains all Angular 8+ Typescript code, assets and basically everything the end user will have access to. |
...
Now that we have installed all prerequisites it's time to start developing our app. Angular-CLI offers a lot of assistance and allows you to generate basically all Angular 8+-Components there are. *(In a smart way!)*

To generate our first `component` we simply open up a terminal and navigate in our Angular 8+-App. Now we simply run `ng g component client` and we get a new component in `/src/app/client` with the following files:
...
These were the first steps with the awesome Angular-CLI and Angular 8+. Following we are going to get more in detail about developing with Angular-CLI and using the power of Angular 8+ right.
...
Receiving data from a server to process or display it in Angular 8+ can be quite tricky with the new observables and syntax. Here you'll find an example how you can do this.
...
* [RxJS](//github.com/ReactiveX/rxjs) - Angular 8+ Dependency - A new approach to how data can be handled in Observables.
